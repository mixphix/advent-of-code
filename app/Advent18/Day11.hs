...
fuelcells =
  NEMop
    . NEMap.fromSet (Sum . powerlevel)
    . relist
    $ join (liftM2 Point2) [1 .. 300]

cumulcells :: NEMop (Point 2 Integer) (Sum Integer)
cumulcells = relist . foldl' f Empty $ join (liftM2 Point2) [1 .. 300]
 where
  f ::
    Map (Point 2 Integer) (Sum Integer) ->
    Point 2 Integer ->
    Map (Point 2 Integer) (Sum Integer)
  f m p@(Point2 x y) =
    insert
      ...
      ( Sum (powerlevel p)
          + m ! Point2 (x - 1) y
          + m ! Point2 x (y - 1) - m ! Point2 (x - 1) (y - 1)
      )
      m
...
subcell w (Point2 x0 y0) =
  let x = x0 - 1
      ...
      [a, b, c, d] =
        getSum . (cumulcells !)
          <$> [Point2 x y, Point2 x y', Point2 x' y, Point2 x' y']
   in d - b - c + a
...
maxsubcumul w =
  withNonEmpty
    (Point2 0 0, 0)
    (maximumOn1 snd)
    [(p, subcell w p) | p <- join (liftM2 Point2) [1 .. 301 - w]]
...
part1 =
  let totalpower pt = getSum $ surrounding 1 pt >-< (fuelcells !)
   in withNonEmpty
        (0, 0)
        (join (***) pred . view pair . maximumOn1 totalpower)
        $ join (liftM2 Point2) [2 .. 299]
...
part2 =
  withNonEmpty
    (0, 0, 0)
    (fst . maximumOn1 snd)
    [((x, y, w), l) | w <- [3 .. 300], let (Point2 x y, l) = maxsubcumul w]

...
step Part2 (a, b) =
  ( find (multipleOf 4) (drop 1 $ iterate' stepA a) ?: 0
  , find (multipleOf 8) (drop 1 $ iterate' stepB b) ?: 0
  )
...
part1 :: Natural
part1 =
  count (uncurry low16Match)
    . take (4 * 10 ^ (7 :: Int))
    . drop 1
    $ iterate' (step Part1) in15

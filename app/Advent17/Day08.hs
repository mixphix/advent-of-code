...
execute i m = case i of
  Increment reg1 qty reg2 cmp
    | testComparing cmp reg2 m ->
      alter (Just . maybe qty (+ qty)) reg1 m
    | otherwise -> m
  Decrement reg1 qty reg2 cmp
    | testComparing cmp reg2 m ->
      alter (Just . maybe (-qty) (subtract qty)) reg1 m
    | otherwise -> m
...
part2 :: Int
part2 =
  withNonEmpty 0 (maximumOf1 (withNonEmpty 0 maximum1)) $
    scanl' (flip execute) Empty in08

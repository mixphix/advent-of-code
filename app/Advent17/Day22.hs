...
virus ::
  Part ->
  (Ant, Map (Point 2 Integer) Char) ->
  (Ant, Map (Point 2 Integer) Char)
virus Part1 (a@(view position -> p), m) = case m !? p of
  Just '#' -> (scurry 1 $ turnR a, m & at p ?~ '.')
  _ -> (scurry 1 $ turnL a, m & at p ?~ '#')
virus Part2 (a@(view position -> p), m) = case m !? p of
...
in22 =
  Map.mapKeys (dihedralTransform TR2 . fmap fromIntegral)
    . view grid
    . map toString
    $ lines (input 2017 22)
...
start =
  Ant North
    . fmap (`div` 2)
    . withNonEmpty origin (maximumOn1 (manhattan origin))
    $ keys in22
